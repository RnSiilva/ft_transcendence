...
#    - set strong secrets before delivery: JWT_SECRET (openssl rand -hex 32),
...
- **Frontend pages & components** (Thiago + AI): implementation of the
  approved visual design fixes, the pages i18n dictionary (PT/EN/ES) and
...
- **Organisation:** [FILL — ex.: feature branches por membro com PRs para
  `main` e revisão cruzada; reuniões semanais; divisão frontend/backend por
  domínios.]
...
| --- | --- | --- | --- |
| Framework frontend **and** backend | Major | 2 | all (React + Express) |
| Real-time features (WebSockets) | Major | 2 | Renan, Pedro |
...
| Remote players (latency, disconnects, reconnection) | Major | 2 | Renan, Pedro |
| Standard user management (profile, avatar, friends, status) | Major | 2 | Carlos, Pedro |
| ORM (Prisma) | Minor | 1 | Renan, Carlos, Pedro |
| Multiple languages (3, full UI, switcher) | Minor | 1 | Thiago |
| **Total core** | | **14** | |
...
| --- | --- | --- | --- |
| Multiplayer game (3+ players) | Major | 2 | up to 6 per room, synced |
...
Additional modules are implemented beyond the +5 cap (game statistics &
match history; a REST API surface). The subject caps bonus at 5 points, so
these only widen the choice of which five to defend; anything claimed is
fully functional and meets its full module description.
